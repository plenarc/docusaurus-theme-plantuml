...
import type { DocusaurusPluginPlantUmlOptions } from './types';
 ...
 */
function plantUMLRemarkPlugin(serverUrl: string) {
...
export default function plantUMLPlugin(
  context: LoadContext,
  options: DocusaurusPluginPlantUmlOptions,
): Plugin | null {
      ...
      if (config.module?.rules) {
        for (const rule of config.module.rules) {
          if (
            ...
            const processUseEntry = (useEntry: any) => {
              if (
...
export type { DocusaurusPluginPlantUmlOptions };
